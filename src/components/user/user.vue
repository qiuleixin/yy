    ...
    <div>
        ...
        <el-card class="box-card">
            ...
            <el-dialog
                ...
                <el-form :model="editRuleForm" :rules="editRules" ref="ruleFormRef" label-width="70px">
...
export default {
    data() {
      ...
      cb(new Error('请输入合法的邮箱'))
        ...
        // 验证手机号的规则
        var checkModeil=(rule,value,callback)=>{
            // 验证手机号的正则表达式
            const regModeil= /^1([38][0-9]|4[579]|5[0-3,5-9]|6[6]|7[0135678]|9[89])\d{8}$/
    ...
    methods: {
        ...
        async editUserInfo(){
            ...
            {email:this.editRuleForm.email,mobile:this.editRuleForm.mobile})
            // console.log(res)
        ...
        async removeUserById(id) {
            // 删除前需要确认消息提示
            const confirmquest=await this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        }).catch(err=>err)
        console.log(confirmquest )
        // 如果用户确认了删除，则返回字符串 confirm
        //如果用户取消了删除，则返回字符串 cancel
        if(confirmquest!='confirm') return this.$message.error('已取消删除')
            // 删除单个用户
